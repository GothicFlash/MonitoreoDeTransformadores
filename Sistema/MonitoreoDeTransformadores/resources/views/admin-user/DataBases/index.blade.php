...
@section('title-content','Bases de datos')
@section('content')
@if ($monitors->first()->transformers->first()->registers->count()>0)
  <form action="{{ route('create-excel') }}" method="post" class="form-horizontal">
    {{ csrf_field() }}
    ...
    <div class="col-md-12">
      <button type="submit" class="btn btn-primary center-block"><i class="fa fa-download fa-fw"></i> Descargar</button>
    </div>
  </form>
@else
  <div class="alert alert-warning"> <strong>Lo sentimos!</strong> No hay bases de datos para generar</div>
@endif
@endsection
@section('script')
