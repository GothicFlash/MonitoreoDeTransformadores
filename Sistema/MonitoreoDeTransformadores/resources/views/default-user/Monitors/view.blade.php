...
<div class="well">
  ...
  <div class="btn-group">
    @foreach ($monitor->transformers()->orderBy('pivot_created_at', 'asc')->get() as $transformer)
      <a href="{{ url('monitor',['monitor' => $monitor->id, 'transformer' => $transformer->id]) }}" onclick="" class="btn btn-info"> Transformador {{ $transformer->id }} </a>
    @endforeach
  </div>
</div>
...
@section('script')
  ...
  <script>getGraphicByProbability({{ $monitor->id }}, {{ $idTransformer }}, "month");</script>
@endsection
